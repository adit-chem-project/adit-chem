ADIT 0.0.1 が生成した Psi4 の計算ディレクトリです (2026-09-13 06:20 (UTC+09:00))
...
== 実行する前に用意すること ==
  ...
  参照波動関数 (reference) を書いていないので、Psi4 の既定 (RHF / RKS) になります。
...
== 研究室のクラスタで実行したいとき ==
  ...
  2. クラスタにログインして、写したディレクトリへ移動します (ssh = 別の計算機にログインするコマンド)
       ssh <ユーザー名>@<クラスタのホスト名>
       cd <クラスタでの作業ディレクトリ>/psi4_h2o_generated
  ...
  5. 終わったら結果を手元に取り戻します (手元の PC のターミナルで)
       scp -r <ユーザー名>@<クラスタのホスト名>:<クラスタでの作業ディレクトリ>/psi4_h2o_generated <手元の置き場所>/
     または
       rsync -av <ユーザー名>@<クラスタのホスト名>:<クラスタでの作業ディレクトリ>/psi4_h2o_generated <手元の置き場所>/

== 結果の見方 ==
  final.xyz     計算に使った最後の構造 (最適化ならその結果)
  output.log    Psi4 の出力。最後に「adit-psi4: done」と結果の要約が入ります
  results.json  入力の最後の数行が書く結果 (エネルギー [Hartree]、Psi4 のバージョン)
...
== 作成時の記録 ==
  ADIT 0.1.0a1 / Python 3.14.7 / ASE 3.29.0 / 生成 2026-09-19T01:29:41+00:00 (UTC)
