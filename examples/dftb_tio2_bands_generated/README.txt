ADIT 0.0.1 が生成した DFTB+ の計算ディレクトリです (2026-09-10 18:22 (UTC+09:00))

計算の種類: バンド計算 / 元素: Ti O / 原子数: 6
計算コード: DFTB+
...
== このディレクトリのファイル ==
  ...
  skf/          Slater-Koster ファイル (セット mio-ext。DFTB+ が使う、元素の組ごとのパラメータ) と LICENSE、README: LICENSE, O-O.skf, O-Ti.skf, README, Ti-O.skf, Ti-Ti.skf
  bands/        バンド計算の 2 段階目。1 段階目の電荷を読み、高対称点を結ぶ経路 (bands/kpath.json) に沿って計算します
...
== 研究室のクラスタで実行したいとき ==
  ...
  2. クラスタにログインして、写したディレクトリへ移動します (ssh = 別の計算機にログインするコマンド)
       ssh <ユーザー名>@<クラスタのホスト名>
       cd <クラスタでの作業ディレクトリ>/dftb_tio2_bands_generated
  ...
  5. 終わったら結果を手元に取り戻します (手元の PC のターミナルで)
       scp -r <ユーザー名>@<クラスタのホスト名>:<クラスタでの作業ディレクトリ>/dftb_tio2_bands_generated <手元の置き場所>/
     または
       rsync -av <ユーザー名>@<クラスタのホスト名>:<クラスタでの作業ディレクトリ>/dftb_tio2_bands_generated <手元の置き場所>/
...
== 結果の見方 ==
  ...
  bands/band.out  経路上の各 k 点のエネルギー準位 (図は ADIT の解析タブか analyze.py で描けます)
...
== 作成時の記録 ==
  ADIT 0.1.0a1 / Python 3.14.7 / ASE 3.29.0 / 生成 2026-09-19T05:32:35+00:00 (UTC)
  パラメータなどのファイルの SHA-256 (中身から計算する照合用のハッシュ。同じ値なら同じファイル。spec.json の provenance にも同じもの):
    ...
    0287863edec03966dacbf7644ad082b6cc170f23f13bef1104f209d9a5f75cd4  skf/O-Ti.skf
    abb2a2a5a5b4e03d50521858b08459c5199a3cad34f99a72ffdeb8fa33ab0166  skf/README
    d65ddc9fa57829378d740a70fae455467d296985f8810a65300b981a3ee0949a  skf/Ti-O.skf
    6d3c24e10103fd11268f073d73cc3979bf3f412fb27d866f55b3d0cf75211ed2  skf/Ti-Ti.skf
