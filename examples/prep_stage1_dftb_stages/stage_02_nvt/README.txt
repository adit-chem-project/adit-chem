ADIT 0.0.1 が生成した DFTB+ の計算ディレクトリです (2026-09-12 02:59 (UTC+09:00))
...
== 段階 ==
  3 段階のうち 2 段階目 (nvt: MD (NVT、300 K、40 ステップ × 0.5 fs、熱浴 berendsen))。前の段階 ../stage_01_min が終わってから実行します。
  submit.sh は計算の前に、前の段階の出力から構造を前の段階の最終構造に書き換えます (python3 と ../handoff.py を使います)。前の段階の出力が無ければ、そこで止まります。
  速度は引き継ぎません (MD なら新しく初速を作ります)。
  spec.json の構造は 1 段階目の最初の構造のままです。実際の出発点は前の段階の出力です。
...
== 作成時の記録 ==
  ADIT 0.1.0a1 / Python 3.14.7 / ASE 3.29.0 / 生成 2026-09-19T05:32:37+00:00 (UTC)
