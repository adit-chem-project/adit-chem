...
== 比べる計算の組 ==
  この計算は adsorption の組の molecule です (組の説明と式は ../README.txt、ΔE = +1·adsorbed -1·slab -1·molecule)。
  - 非周期なので k 点を使いません
...
== 研究室のクラスタで実行したいとき ==
  ...
  5. 終わったら結果を手元に取り戻します (手元の PC のターミナルで)
       scp -r <ユーザー名>@<クラスタのホスト名>:<クラスタでの作業ディレクトリ>/molecule <手元の置き場所>/
     または
       rsync -av <ユーザー名>@<クラスタのホスト名>:<クラスタでの作業ディレクトリ>/molecule <手元の置き場所>/
