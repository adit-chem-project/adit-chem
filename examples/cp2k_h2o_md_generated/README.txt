ADIT 0.0.1 が生成した CP2K の計算ディレクトリです (2026-09-12 02:13 (UTC+09:00))
...
== 研究室のクラスタで実行したいとき ==
  ...
  2. クラスタにログインして、写したディレクトリへ移動します (ssh = 別の計算機にログインするコマンド)
       ssh <ユーザー名>@<クラスタのホスト名>
       cd <クラスタでの作業ディレクトリ>/cp2k_h2o_md_generated
  ...
  5. 終わったら結果を手元に取り戻します (手元の PC のターミナルで)
       scp -r <ユーザー名>@<クラスタのホスト名>:<クラスタでの作業ディレクトリ>/cp2k_h2o_md_generated <手元の置き場所>/
     または
       rsync -av <ユーザー名>@<クラスタのホスト名>:<クラスタでの作業ディレクトリ>/cp2k_h2o_md_generated <手元の置き場所>/

== 結果の見方 ==
  output.log    CP2K の出力 ('ENERGY| Total FORCE_EVAL' の行が全エネルギー。単位は Hartree)
  adit-pos-1.xyz  MD の軌跡 (dump の間隔ごとの座標、xyz 形式)
  adit-1.ener   各ステップの時刻・運動エネルギー・温度・ポテンシャルエネルギー・保存量
  analyze.py    ADIT の解析タブと同じ処理で、図と要約を analysis/ に書きます。ADIT が入った Python で実行します
       python analyze.py --rdf --msd   (動径分布関数と拡散係数も求めます)

== 作成時の記録 ==
  ADIT 0.1.0a1 / Python 3.14.7 / ASE 3.29.0 / 生成 2026-09-19T01:36:09+00:00 (UTC)
