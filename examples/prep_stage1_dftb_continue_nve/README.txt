ADIT 0.1.0a1 が生成した DFTB+ の計算ディレクトリです (2026-09-19 10:29 (UTC+09:00))
...
== 前の計算から引き継いだもの ==
  前の計算: /home/<user>/adit/examples/dftb_md_water_generated (dftbplus、molecular_dynamics)
  + 位置 (geo_end.xyz から)
  + 速度 (Å/fs に直して spec.json の structure.velocities に)
...
== 研究室のクラスタで実行したいとき ==
  ...
  2. クラスタにログインして、写したディレクトリへ移動します (ssh = 別の計算機にログインするコマンド)
       ssh <ユーザー名>@<クラスタのホスト名>
       cd <クラスタでの作業ディレクトリ>/prep_stage1_dftb_continue_nve
  ...
  5. 終わったら結果を手元に取り戻します (手元の PC のターミナルで)
       scp -r <ユーザー名>@<クラスタのホスト名>:<クラスタでの作業ディレクトリ>/prep_stage1_dftb_continue_nve <手元の置き場所>/
     または
       rsync -av <ユーザー名>@<クラスタのホスト名>:<クラスタでの作業ディレクトリ>/prep_stage1_dftb_continue_nve <手元の置き場所>/
...
== 作成時の記録 ==
  ADIT 0.1.0a1 / Python 3.14.7 / ASE 3.29.0 / 生成 2026-09-19T01:29:55+00:00 (UTC)
