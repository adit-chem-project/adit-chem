...
`--host 0.0.0.0` のように他の PC から届くアドレスで起動すると、起動のたびに合言葉 (トークン) が作られ、端末に `http://…:8765/?token=…` の形で表示されます。この URL を一度開くと、そのブラウザでは以後合言葉なしで使えます。合言葉は `--token` で自分で決めることもでき、`--no-token` で外せます。通信は暗号化されないので、学内などの信頼できるネットワークの中だけで使ってください。
