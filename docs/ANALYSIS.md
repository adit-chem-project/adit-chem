...
「解析」ページでは計算結果のディレクトリを解析し、図と要約をそのページに表示します。要約の下の「3D で再生」で、軌跡や最適化のステップをスライダーと再生ボタンで動かせます (エネルギー・温度の小さなグラフの縦線がフレームに追従し、クリックでそのフレームへ飛びます)。spec.json の読み込みとダウンロードもできます。
